...
def multiple_panel_plot(x,y,
	...
	fs=14,             #font size
	fname='multiple_panel_plot.png'):   #default filename
	
	#by default, flag_save = True, and we'll save the figure 
	#to a file. The plot ffilename fname = 'plot.png' by
	...
	for i in range(nrow):
		for j in range(ncolumn):
			z = y +float(i)
			#plot y vs. x
			ax[i,j].plot(x,z,color=lcolor,linewidth=1.5,label='model (line)')  #a line
			#label our axes
